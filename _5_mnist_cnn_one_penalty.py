...
class DigitCNN():
    ...
    def __init__(self):
        ...
        ones = theano.shared(np.ones(784))
        ...
        row_diff    = T.dot(self.pW,ones) - ones 
        col_diff    = T.dot(self.pW.transpose(), ones) - ones
        ...
        updates += SGD_positive(cost, [self.pW], eta)

        self.shapes = theano.function(inputs=[X, drop_conv],outputs=[l1.shape, l2.shape, l3.shape], allow_input_downcast=True)

        self.train_func = theano.function(inputs=[X, Y, drop_conv, drop_full, row_penalty, col_penalty, eta], outputs=cost, updates=updates, allow_input_downcast=True)
        self.predict_func = theano.function(inputs=[X, drop_conv, drop_full], outputs=y_x, allow_input_downcast=True)

    def train(self, X, Y, drop_conv=.3, drop_full=.5, row_penalty=0.0, col_penalty=0.0, eta=.01):
        return self.train_func(X, Y, drop_conv, drop_full, row_penalty, col_penalty, eta)

import numpy as np
from _5_mnist_cnn_one_penalty import DigitCNN
import ml_datasets
...
def train(iters=1, eta=.1, row_penalty=0.0, col_penalty=0.0):

    for i in xrange(iters):
        for start, end in tqdm(zip(range(0, len(trX), 128), range(128, len(trX), 128))):
            cost = cnn.train(trX[start:end], trY[start:end], row_penalty=row_penalty, col_penalty=col_penalty, eta=eta)
